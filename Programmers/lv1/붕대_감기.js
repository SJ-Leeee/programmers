...
function solution(bandage, health, attacks) {
  ...
  for (let i = 0; i < attacks.length; i++) {
    ...
    if (i !== attacks.length - 1) {
      let bandageTime = attacks[i + 1][0] - attacks[i][0] - 1;
      let bonusHp = Math.floor(bandageTime / bandage[0]);
      state += bonusHp * bandage[2];
      state += bandageTime * bandage[1];
      if (state > health) state = health;
      console.log(`${i}번째 회복 체력: ${state}`);
    }
  }
  return state;
}
